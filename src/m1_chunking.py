...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            # Similarity below threshold -> start new chunk
            ...
            # Similarity above threshold -> continue current chunk
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if len(current_parent_text) + len(para) + 2 <= parent_size:
            current_parent_text += para + "\n\n"
            current_parent_paragraphs.append(para)
        else:
            # Save current parent if not empty
            if current_parent_text.strip():
                ...
                parent_for_children = current_parent_text.strip()
                child_text = ""
                for p in current_parent_paragraphs:
                    if len(child_text) + len(p) + 2 <= child_size:
                        child_text += p + "\n\n"
                    else:
                        if child_text.strip():
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
        ...
        if re.match(r'^#{1,3}\s+', part):
            ...
            if current_content and current_section:
                ...
                chunks.append(Chunk(
                    ...
                    metadata={**metadata, "section": current_section, "strategy": "structure"}
                ))
            ...
            # Content part - append to current
    ...
    if not chunks:
        chunks.append(Chunk(
            text=text.strip(),
            metadata={**metadata, "section": "document", "strategy": "structure"}
        ))
